...
type LetterKind = 'offer' | 'declination';
...
// Who signs the offer letter. Alex has a scanned signature; others sign in a
// script font. The letter body's "contact Zack Lawson…" line is separate.
interface Signatory { key: string; name: string; title: string; img?: string }
const SIGNATORIES: Signatory[] = [
  { key: 'alex', name: 'Alex Little', title: 'Founding & Managing Partner', img: SIG_B64 },
  { key: 'zack', name: 'Zack Lawson', title: 'Founding Partner' },
  { key: 'catie', name: 'Catie Toole', title: 'Director of Operations' },
];
...
export default function OffersClient() {
  ...
  const [signerKey, setSignerKey] = useState('alex');
  const signer = SIGNATORIES.find(s => s.key === signerKey) ?? SIGNATORIES[0];
  ...
  function printPdf() {
    ...
    // Closing block (right side) — always rendered from the selected signatory,
    // so the signature/name/title reflect who is signing (Alex, Zack, or Catie).
    const sigInner = `<div>Very truly yours,</div>`
      + `<div style="height:5pt"></div>`
      + (signer.img
          ? `<div><img src="${signer.img}" width="148" height="49" style="display:block;margin-bottom:1pt" alt=""/></div>`
          : `<div style="font-family:${SCRIPT_FONT};font-size:22pt;line-height:1;margin:2pt 0 3pt">${esc(signer.name)}</div>`)
      + `<div>${esc(signer.name)}</div>`
      + `<div>${esc(signer.title)}</div>`;
      ...
      <header className="px-8 py-5 bg-white border-b border-border flex-shrink-0 flex items-center justify-between gap-4">
        <div>
          <h1 className="font-spectral text-[23px] font-semibold text-text-primary">
            {letterKind === 'offer' ? 'Offer Letters' : 'Declination Letters'}
          </h1>
        ...
        <div className="grid grid-cols-[360px_1fr] gap-6 max-w-5xl items-start">
          ...
          <div className="bg-white border border-border rounded-card p-6 space-y-4 sticky top-0">
            ...
            <div>
              ...
              <select value={signerKey} onChange={e => setSignerKey(e.target.value)}
                className="w-full border border-border-light rounded-ctrl px-3 py-2 text-sm bg-white focus:outline-none focus:border-ink">
                {SIGNATORIES.map(s => <option key={s.key} value={s.key}>{s.name} — {s.title}</option>)}
              </select>
            </div>
          ...
          <div className="space-y-3">
            <div className="bg-white border border-border rounded-card overflow-hidden shadow-sm">
              ...
              {/* Closing block — right-aligned, reflects the selected signatory */}
              {draft && /very truly yours/i.test(draft) && (
                <div className="px-8 pb-5 flex justify-end">
                  <div className="text-left">
                    ...
                    {signer.img ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={signer.img} alt={`${signer.name} signature`} width={130} height={43} className="block my-0.5" />
                    ) : (
                      <div className="text-[28px] leading-none text-text-primary my-1" style={{ fontFamily: SCRIPT_FONT }}>{signer.name}</div>
                    )}
                    <p className="text-[13px] leading-snug" style={{ fontFamily: BODY_FONT }}>
                      {signer.name}<br />{signer.title}<br />Litson PLLC
                    </p>
                  </div>
                </div>
              )}
