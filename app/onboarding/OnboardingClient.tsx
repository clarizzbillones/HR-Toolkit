...
export default function OnboardingClient() {
  ...
  const [view, setView] = useState<'dashboard' | 'guides' | 'intake' | 'workflow' | 'emails' | 'agenda'>('dashboard');
  ...
  useEffect(() => { try { const t = new URLSearchParams(window.location.search).get('tab'); if (t && ['dashboard', 'workflow', 'guides', 'intake', 'emails', 'agenda'].includes(t)) setView(t as any); } catch { /* ignore */ } }, []);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <header className="px-8 py-5 bg-white border-b border-border flex-shrink-0 flex items-center gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-text-muted mt-0.5">{view === 'dashboard' ? 'Track each new hire’s progress; completed people flow into Staffing' : view === 'intake' ? 'Share a link for future hires to fill out their info and upload documents' : view === 'workflow' ? 'The standard hiring & onboarding journey, interview to first check-ins' : view === 'emails' ? 'Ready-to-send emails for new hires — copy and paste into Outlook' : view === 'agenda' ? 'Onboarding call agendas — personalize and download as PDF or Word' : 'Edit, add, or remove anything, then send it'}</p>
        </div>
        {canSeeModule && (
        <div className="flex items-center bg-[#f1ece3] rounded-ctrl p-0.5 ml-4">
          {(['dashboard', 'workflow', 'guides', 'intake', 'emails', 'agenda'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              ...
              {v === 'dashboard' ? 'Dashboard' : v === 'workflow' ? 'Workflow' : v === 'guides' ? 'Guide Templates' : v === 'intake' ? 'Intake Links' : v === 'emails' ? 'Email Templates' : 'Agenda'}
            </button>
          ))}
        </div>
        )}
