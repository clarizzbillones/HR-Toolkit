...
import { parseDoc } from '@/lib/onboardingDoc';
...
// Email each assignee the incomplete onboarding-document tasks assigned to them.
export async function POST(req: Request) {
  const session = await getServerSession(authOptions);
  if (!session?.user) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  const { id } = await req.json();
  if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 });
  const [row] = await sql`SELECT name, doc FROM onboardees WHERE id = ${id}` as any[];
  ...
  for (const r of [...doc.hr, ...doc.accounts, ...doc.it]) {
    ...
    byAssignee.get(a)!.push({ label: r.label, deadline: r.cell?.deadline });
  }
  ...
  const { sent, skipped } = await notifyAssignees({ employeeName: row.name, kindLabel: 'Onboarding', groups, appUrl: origin(req) });
