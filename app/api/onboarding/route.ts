...
const SEED_VERSION = 4; // bump to force a one-time reseed of the templates
...
const G_LINKS = [
  ...
  'Zoom', 'Ajax (timekeeping)', 'Donna (internal AI assistant)', 'Employee Forms Website', 'Company SOPs',
  'Company Directory', 'Company Handbook (via ADP)', 'Westlaw', 'PACER', 'Litson Availability Calendar',
  'PTO and Leave Request Procedure', 'IT Ticket Request Procedure', 'Reporting Phishing Emails in Outlook',
];
...
async function seedGuide(guide: string, sections: [number, string, string][], schedule: [string, string, string, string, string][], links: string[], tasks: [string, string][]) {
...
async function seedAll() {
  await seedGuide('General', G_SECTIONS, G_SCHEDULE, G_LINKS, G_TASKS);
  await seedGuide('Attorney', A_SECTIONS, [], A_LINKS, A_TASKS);
}
...
export async function POST(req: Request) {
  ...
  const kind = ['section', 'schedule', 'sop', 'task'].includes(body.kind) ? body.kind : 'section';
