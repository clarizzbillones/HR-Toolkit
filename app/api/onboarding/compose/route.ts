...
export async function GET() {
  try {
    ...
    const composed: { name: string; sources: string[] }[] = [];
    for (const r of rows) {
      if (!r.guide) continue;
      try { const s = JSON.parse(r.body); if (Array.isArray(s)) composed.push({ name: r.guide, sources: s.map(String) }); } catch { /* skip */ }
    }
...
export async function POST(req: Request) {
  ...
  const body = JSON.stringify(sources.map(String));
