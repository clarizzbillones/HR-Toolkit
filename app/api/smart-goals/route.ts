...
function parse(r: any) { return { ...r, goals: parseGoals(r.goals), open_items: parseItems(r.open_items) }; }
...
export async function POST(req: Request) {
  ...
  const id = cuid();
  await sql`INSERT INTO smart_goals (id, employee, employee_email, reviewer, reviewer_position, review_date, goals_prepared, milestones, goals, open_items, status)
    VALUES (${id}, ${b.employee ?? ''}, ${b.employee_email ?? ''}, ${b.reviewer ?? ''}, ${b.reviewer_position ?? ''}, ${b.review_date ?? null}, ${b.goals_prepared ?? null}, ${b.milestones ?? ''}, ${JSON.stringify(b.goals ?? [])}, ${JSON.stringify(b.open_items ?? [])}, ${b.status ?? 'Draft'})`;
