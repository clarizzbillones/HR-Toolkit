...
const FIELDS = ['name','position','dialpad','personal_phone','email','start_date','dob','favorite_color','favorite_treat','note','ktn','marriott','delta','southwest','offboarded'] as const;
...
export async function POST(req: Request) {
  ...
  for (const r of clean) {
    ...
    await sql`INSERT INTO offboarded_staff (id,name,position,dialpad,personal_phone,email,start_date,dob,favorite_color,favorite_treat,note,ktn,marriott,delta,southwest,offboarded)
      VALUES (${rid()},${v.name},${v.position},${v.dialpad},${v.personal_phone},${v.email},${v.start_date},${v.dob},${v.favorite_color},${v.favorite_treat},${v.note},${v.ktn},${v.marriott},${v.delta},${v.southwest},${v.offboarded})`;
  }
  const rows = await sql`SELECT * FROM offboarded_staff ORDER BY name ASC`;
...
export async function PATCH(req: Request) {
  ...
  await sql`UPDATE offboarded_staff SET ${sql(updates)} WHERE id = ${id}`;
  const [row] = await sql`SELECT * FROM offboarded_staff WHERE id = ${id}`;
...
export async function DELETE(req: Request) {
  ...
  if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 });
  await sql`DELETE FROM offboarded_staff WHERE id = ${id}`;
