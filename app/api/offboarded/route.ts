...
const FIELDS = ['name','position','dialpad','personal_phone','email','start_date','dob','favorite_color','favorite_treat','note','ktn','marriott','delta','offboarded'] as const;
...
export async function POST(req: Request) {
  ...
  for (const r of clean) {
    ...
    await sql`INSERT INTO offboarded_staff (id,name,position,dialpad,personal_phone,email,start_date,dob,favorite_color,favorite_treat,note,ktn,marriott,delta,offboarded)
      VALUES (${rid()},${v.name},${v.position},${v.dialpad},${v.personal_phone},${v.email},${v.start_date},${v.dob},${v.favorite_color},${v.favorite_treat},${v.note},${v.ktn},${v.marriott},${v.delta},${v.offboarded})`;
  }
  const rows = await sql`SELECT * FROM offboarded_staff ORDER BY name ASC`;
