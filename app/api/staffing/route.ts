...
// Self-heal: collapse duplicate Staffing rows for the same person into one.
// Rows are grouped by a nickname-tolerant name key (William "Bill" Abely ==
// William Abely), a non-offboarded row is kept, its blank columns (and custom
// "extra" fields) are filled from the others, then the extras are deleted.
// Idempotent — once collapsed, each group has a single row. Runs on load so
// duplicates clear themselves without any manual merge step.
...
async function autoMergeDuplicateStaff() {
  try {
