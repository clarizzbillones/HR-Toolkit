...
// §9 — scheduling. Next review = 6 months after the last review (rolling);
// first review = 6 months after hire when never reviewed.
const cases: [string, string, string | null, string, number, string][] = [
  ['Never reviewed', '2026-03-02', null, '2026-09-02', 1, '0.5 yr'],
  ['Early review (grace month)', '2025-04-21', '2025-10-15', '2026-04-15', 2, '1.0 yr'],
  ['On time', '2025-12-30', '2026-07-02', '2027-01-02', 2, '1.0 yr'],
  ['Past 1 yr — must keep going', '2024-05-20', '2026-05-28', '2026-11-28', 5, '2.5 yr'],
  ['Late review, no drift', '2024-08-15', '2026-05-07', '2026-11-07', 4, '2.0 yr'],
  ['Month-end clamp (never reviewed)', '2024-08-31', null, '2025-02-28', 1, '0.5 yr'],
  ['Month-end clamp (last review)', '2025-05-20', '2025-08-31', '2026-02-28', 1, '0.5 yr'],
];
...
{
  const t = '2026-07-16';
  // Next = 6 months after last review; ~66d overdue stays Overdue (within window).
  const fer = computeReview('2025-02-25', '2025-11-11', t); // next = 2026-05-11
  eq('roll · Fer next', fer.next, '2026-05-11');
  eq('roll · Fer cycle', fer.cycle, 2);
  eq('roll · Fer tenure', fer.tenure, '1.0 yr');
  eq('roll · Fer status', fer.status, 'Overdue');
  ...
  // Deeply missed: last+6mo far past → rolls forward 6mo at a time into reach.
  const missed = computeReview('2024-01-01', '2024-06-01', t); // base 2024-12-01
  eq('roll · missed advances', missed.next, '2026-06-01');
}
